...
function jira_call() {
    ...
    CURL=$(curl --location --silent --request POST -u "$JIRA_USER_EMAIL:$JIRA_USER_API_KEY" --header "Content-Type: application/json" "$JIRA_WORKSPACE_URL"/rest/api/2/search -d '{"jql": "assignee=currentuser() AND status NOT IN (Done, Rejected)", "maxResults":50}')
...
if [[ -n $TICKET ]]; then
    ...
    if [[ -n "$TICKET_NUMBER" ]] && [[ -n "$DESC" ]]; then
        gt branch create "$TICKET_NUMBER/$DESC"
    fi
fi
